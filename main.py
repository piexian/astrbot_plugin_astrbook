...
ASTRBOOK_MAX_IMAGE_BYTES = 10 * 1024 * 1024
...
class AstrbookPlugin(Star):
    ...
    async def _read_limited_response(
        ...
        *,
        max_bytes: int = ASTRBOOK_MAX_IMAGE_BYTES,
    ) -> bytes:
        ...
        if isinstance(errors, list):
            ...
            for item in errors:
                if isinstance(item, dict):
                    message = AstrbookPlugin._extract_error_message(item)
                    if message:
    ...
    def _extract_image_url(payload: Any) -> str | None:
        if isinstance(payload, list):
            for item in payload:
                url = AstrbookPlugin._extract_image_url(item)
                if url:
                    return url
            return None
        ...
        for key in ("image_url", "url", "src", "publicUrl"):
            value = payload.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
    ...
    def _moderation_notice(
        payload: Any, *, async_review_possible: bool = False
    ) -> str:
        if isinstance(payload, dict) and "moderated" in payload:
            if payload.get("moderated") is False:
                return (
                    "\nModeration: submitted and pending review. If rejected, "
                    "check_notifications(fetch_details=true) will show the reason."
                )
            return ""

        if async_review_possible:
            return (
                "\nModeration: if the forum rejects this later, "
                "check_notifications(fetch_details=true) will show the reason."
            )
    ...
    async def _parse_error_response(self, resp: aiohttp.ClientResponse) -> str:
        ...
        if message:
            return message[:200]
        return str(payload)[:200] if payload else "No response"
    ...
    def _build_active_send_failure_repair_prompt(reason: str | None) -> str:
        ...
        return (
            ...
            'reply_thread(thread_id=..., content="your complete reply text"), '
            'reply_floor(reply_id=..., content="your complete reply text"), or '
            'send_dm_message(target_user_id=..., content="your complete message text"). '
            "The content argument is required and must not be omitted or empty. "
            "Never call these tools with only an ID. "
        ...
        req.extra_user_content_parts.append(
            TextPart(
                text=event.get_extra(
                    ...
                    "text. You must call the relevant AstrBook tool with every "
                    "required argument. For replies, use "
                    'reply_thread(thread_id=..., content="your complete reply text") '
                    "or "
                    'reply_floor(reply_id=..., content="your complete reply text"). '
                    "The content argument is required and must not be omitted or empty.",
                )
            ).mark_as_temp()
        )
    ...
    @filter.on_agent_done(priority=-100)
    async def signal_astrbook_browse_done(
        self,
        event: AstrMessageEvent,
        run_context: ContextWrapper[AstrAgentContext],
        resp: LLMResponse,
    ):
        """Signal AstrBook adapter when a browse event finishes."""
        try:
            if not event.get_extra("is_browse_event", False):
                return
            adapter = getattr(event, "adapter", None)
            notify = getattr(adapter, "notify_browse_done", None)
            if notify:
                notify(event.get_extra("browse_event_id"))
        except Exception as e:
            logger.warning(f"[astrbook] Failed to signal browse completion: {e}")
    ...
    async def _parse_response(self, resp: aiohttp.ClientResponse) -> dict:
        ...
        if 200 <= resp.status < 300:
            ...
            error = await self._parse_error_response(resp)
            return {
                "error": f"Request failed: {resp.status} - {error}"
            }
    ...
    async def create_thread(
        ...
        if "id" in result:
            event.set_extra("astrbook_tool_reply_sent", True)
            moderation = self._moderation_notice(result, async_review_possible=True)
            return (
                f"Thread created! ID: {result['id']}, Title: {result['title']}"
                f"{moderation}"
            )

        event.set_extra("astrbook_tool_reply_sent", True)
        moderation = self._moderation_notice(result, async_review_possible=True)
        return f"Thread created successfully{moderation}"
    ...
    async def reply_thread(self, event: AstrMessageEvent, thread_id: int, content: str):
        ...
        event.set_extra("astrbook_tool_reply_sent", True)
        moderation = self._moderation_notice(result, async_review_possible=True)
        return f"Reply successful{moderation}"
    ...
    async def reply_floor(self, event: AstrMessageEvent, reply_id: int, content: str):
        ...
        event.set_extra("astrbook_tool_reply_sent", True)
        moderation = self._moderation_notice(result, async_review_possible=True)
        return f"Sub-reply successful{moderation}"
    ...
    async def check_notifications(
        ...
        """
        forum_count, dm_count = await asyncio.gather(
            self._make_request("GET", "/api/notifications/unread-count"),
            self._make_request("GET", "/api/dm/unread-count"),
        )
        ...
        detail_tasks = {
            "forum": self._make_request(
                "GET",
                "/api/notifications",
                params={"page_size": 10, "is_read": "false"},
            )
        }
        if dm_error is None and dm_unread > 0:
            detail_tasks["dm"] = self._make_request(
                "GET", "/api/dm", params={"page": 1, "page_size": 20}
            )
        detail_results = await asyncio.gather(*detail_tasks.values())
        details = dict(zip(detail_tasks.keys(), detail_results, strict=False))
        forum_list = details["forum"]
        if "error" in forum_list:
            ...
            if forum_items:
                ...
                for n in forum_items:
                    ...
                    content_limit = 160 if raw_type == "moderation" else 50
                    content = (n.get("content_preview") or "")[:content_limit]

                    lines.append(f"  {ntype} from @{username}")
                    if thread_id or thread_title:
                        lines.append(f"   Thread: [{thread_id}] {thread_title}")
                    if reply_id:
                        lines.append(f"   Reply ID: {reply_id}")
                    lines.append(
                        f"   {'Notice' if raw_type == 'moderation' else 'Content'}: {content}"
                    )
                    if raw_type == "moderation":
                        lines.append(
                            "   -> Moderation result; revise and repost if needed."
                        )
                    else:
                        lines.append(
                            f"   → To respond: reply_floor(reply_id={reply_id}, content='your complete reply text')"
                            if reply_id
                            else f"   → To respond: reply_thread(thread_id={thread_id}, content='your complete reply text')"
                        )
                    lines.append("")
            else:
        ...
        if dm_error:
            ...
            if "error" in dm_list:
                ...
                if unread_items:
                    ...
                    lines.append(
                        'Use send_dm_message(target_user_id=..., content="your complete message text") to reply. '
                        "The content argument is required and must not be omitted or empty."
                    )
                else:
                    lines.append(
    ...
    async def upload_image(self, event: AstrMessageEvent, image_source: str):
        ...
        try:
            if is_url:
                # Download from URL
                session = await self._get_http_session()
                async with session.get(image_source, timeout=timeout) as resp:
                    if resp.status != 200:
                ...
                if os.path.getsize(image_source) > ASTRBOOK_MAX_IMAGE_BYTES:
                    return "Error: Image too large (>10MB). Cannot upload."
            ...
            async with session.post(
                upload_url, headers=headers, data=form, timeout=timeout
            ) as resp:
                if resp.status == 200:
                    ...
                    error = await self._parse_error_response(resp)
                    return f"Upload failed: {error or 'Token invalid or expired'}"
                elif resp.status == 429:
                    error = await self._parse_error_response(resp)
                    return f"Upload failed: {error or 'Daily upload limit reached, please try again tomorrow'}"
                else:
                    error = await self._parse_error_response(resp)
                    return f"Upload failed: {resp.status} - {error}"
    ...
    async def save_forum_diary(self, event: AstrMessageEvent, diary: str):
        ...
        try:
            adapter = self._get_astrbook_adapter()
            if not isinstance(adapter, AstrBookAdapter):
                return "未找到 AstrBook 适配器实例，暂时无法保存论坛日记。"

            content = diary.strip()
            adapter.memory.add_diary(
                content,
                metadata={
    ...
    async def recall_forum_experience(self, event: AstrMessageEvent, limit: int = 5):
        ...
        try:
            adapter = self._get_astrbook_adapter()
            if not isinstance(adapter, AstrBookAdapter):
                return "未找到 AstrBook 适配器实例，暂时无法回忆论坛经历。"

            diaries = adapter.memory.get_diaries(limit=max(1, int(limit)))
    ...
    async def share_thread(self, event: AstrMessageEvent, thread_id: int):
        ...
        try:
            ...
            with open(tmp_path, "wb") as f:
                f.write(image_data)
            track_temp = getattr(event, "track_temporary_local_file", None)
            if track_temp:
                track_temp(tmp_path)
            ...
            return f"截图过大，帖子链接: {share_link}，原因: {e}"
        except Exception as e:
